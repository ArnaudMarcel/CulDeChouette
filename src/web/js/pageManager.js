...
function montrerDesLanceur(msg) {
    console.log(msg.lancer.interaction);
    document.getElementById("des1").src = "img/" + msg.lancer.valeurDes1 + ".png";
    document.getElementById("des2").src = "img/" + msg.lancer.valeurDes2 + ".png";
    document.getElementById("des3").src = "img/" + msg.lancer.valeurDes3 + ".png";
    document.getElementById("des1").className = "fadeIn load";
...
function loadCreation() {
    ...
    document.getElementById('creation').addEventListener('click', event => {
        console.log(event.target.value);
        switch (event.target.value) {
            case 'Annuler':
                loadIndex();
            ...
            <div id="menu">
                <ul>
                    <li>
                        ...
                        <a id="historique">Historique</a><br><br>
                        <a id="modifier">Modifier le compte</a><br><br>
...
function loadModifications() {
    ...
    <center>
        <img src="img/logo.png" alt="logo.png" id="logoCreation">
        <div id="modifs">
            <h1>Changer les informations de votre compte</h1>
            <form method="post">
                <label for="motDePasse">Mot de passe :</label>
                <input id="motDePasse" type="password" required><br><br>
                <label for="sexe">Sexe :</label>
    ...
    document.getElementById('modifs').addEventListener('click', event => {
        console.log(event.target.value);
        switch (event.target.value) {
            case 'Annuler':
                loadIndexConnected();
                break;
            case 'Valider':
                let pseudo = CDCjoueur.getPseudo();
                let mdp = document.getElementById('motDePasse').value;
                let ville = document.getElementById('ville').value;
                let ageJ = document.getElementById('age').value;
                CDCjoueur = new Joueur(pseudo);
                ...
                if (sexe != null && ageJ != '' && pseudo != '' && mdp != '' && ville != '') {
                    event.stopImmediatePropagation();
                    CDCsocket._sendModifications(pseudo, mdp, sexe, ville, ageJ);
                }
                break;
        }
    });
...
function loadHistorique(data) {
    ...
    `<tr><th>Numéro de partie</th><th>Points</th><th></th><tr>` + data.parties.map(p => {
        let gagnant = p.estGagnant ? 'Victoire' : 'Défaite';
        return `<tr>
            <td>Partie #${p.idPartie}</td>
            <td>${p.nbPoints}</td>
            <td>${gagnant}</td>
        <td>`;
    }).join('');
...
let pageManager = document.addEventListener('click', event => {
    switch (event.target.id) {
        ...
        case 'historique':
            CDCjoueur != null ?
                CDCsocket._historique(CDCjoueur.getPseudo()) : '';
            break;
