class CDCsocket {
    ...
    static listeDesJoueurs(msg) {
        ...
        msg.joueursDisp.forEach(elt => {
            JoueursInv += `<tr>
                ...
                <td>
                    <button name="${elt}" class="inviter" id="buttonInvit">Inviter</button>
                </td>
                </tr>`;
        });
    ...
    static PartieSupprimee() {
        ...
        Swal.fire({
            icon: 'info',
            html: '<h2 style="font-weight:lighter; font-size:23px;">Partie annulée...</h2><br><p>Le groupe de la partie a été dissout.</p>',
            confirmButtonColor: 'rgb(0, 151, 0)',
        });
    }
    ...
    static PartieTerminee(msg) {
        Swal.fire({
            html: `<h2 style="font-weight:lighter; font-size:23px;">Partie Terminée !</h2><br><p>Le gagnant est ${msg.gagnant}</p>`,
            confirmButtonColor: 'rgb(0, 151, 0)',
        }).then(() => {
            loadIndexConnected();
        });
    ...
    static updateScores(msg) {
        console.log(msg);
        UpdateScore(msg.score);
        UpdateBottomScore(msg.score);
    }

    static _sendModifications(pseudo, mdp, sexe, ville, ageJ) {
        this.service.send(JSON.stringify({
            id: "modificationJoueur",
            pseudoJoueur: pseudo,
            motDePasseJoueur: mdp,
            sexeJoueur: sexe,
            villeJoueur: ville,
    ...
    static changements_reussis(msg) {
        loadIndexConnected();
        Swal.fire({
            icon:'success',
            html: `<h2 style="font-weight:lighter; font-size:23px;">Modifications réussies</h2>`,
        });
    }

    static _historique(pseudo) {
        this.service.send(JSON.stringify({
            id: 'historiqueJoueur',
            pseudoJoueur: pseudo
        }));
